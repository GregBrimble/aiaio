...
describe("getSecurityHeaders", () => {
	it("returns security headers with the provided nonce", () => {
		const nonce = "abc123";
		const headers = getSecurityHeaders(nonce);

		expect(headers).toMatchInlineSnapshot(`
			{
			  "Content-Security-Policy": "base-uri 'self'; default-src 'self'; frame-ancestors 'none'; img-src 'self' data:; object-src 'none'; script-src 'self' 'nonce-abc123'",
