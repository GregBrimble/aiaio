...
describe("@aviation/context", () => {
	...
	describe("default context", () => {
		for (const [name, getter] of Object.entries({
			...
			describe(name, () => {
				test("basic context value", ({ expect }) => {
				...
				test("context doesn't leak across children", async ({ expect }) => {
				...
				test("context doesn't leak across siblings", async ({ expect }) => {
					// set a ------------------------------ get a
					// ----------- set b ------ get b -----------
		...
		describe("runWithContext(value, callback)", () => {
			test("sets contexts and a default AsyncLocalStorage", ({ expect }) => {
			...
			test("does not leak the context value out of the callback", ({
				expect,
			}) => {
				const value = Symbol("value");
		...
		describe("getContext()", () => {
			test("returns the default context value set by `runWithContext(value, callback)`", ({
				expect,
			}) => {
				const value = Symbol("value");
			...
			test("throws an error when invoked outside of a context", ({
				expect,
			}) => {
				expect(() => getContext()).toThrowError(ContextNotSetError);
	...
	describe("runWithContext(key, value, callback)", () => {
		test("sets an AsyncLocalStorage for that key", ({ expect }) => {
		...
		test("nests with other values and contexts", async ({ expect }) => {
	...
	describe("getContext(key)", () => {
		test("returns the key'd context value set by `runWithContext(key, value, callback)`", ({
			expect,
		}) => {
			const context = Symbol("key");
			const value = Symbol("value");
		...
		test("throws an error when invoked outside of a context", ({ expect }) => {
			const context = Symbol("key");

			expect(() => getContext(context)).toThrowError(ContextNotSetError);
		...
		test("works with Symbol.for(key)", ({ expect }) => {
			const value = Symbol("value");
	...
	describe("in a React render", () => {
		test("context passes through to children from root", ({ expect }) => {
			...
			const ParentComponent = () => {
				...
				return runWithContext(value, () => {
					return <ChildComponent />;
				});
			};

			expect(
				runWithContext("Set from root", () =>
					renderToString(<ParentComponent />),
				),
			).toMatchInlineSnapshot(`"<p>Set from root</p>"`);
		});

		test("context passes through to children from parents when using render props", ({
			expect,
		}) => {
			const ChildComponent = () => {
				return <p>{getContext()}</p>;
		...
		test("context can be manipulated by rendering component", ({ expect }) => {
			interface Context {
				value: string;
			}
			...
			const returnValue = runWithContext(context, () =>
				renderToString(<ChildComponent />),
			);
			expect(returnValue).toMatchInlineSnapshot(`"<p>Child component</p>"`);
